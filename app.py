...
def recalculate(df: pd.DataFrame, diameter: float, gradient: float, flow_per_pe: float, n: float = DEFAULT_N) -> pd.DataFrame:
    new_df = df.copy()
    new_df["Index"] = range(1, len(new_df) + 1)
    new_df["Design_Flow_m3s"] = new_df["PE_on_Line"] * flow_per_pe * new_df["Peak_Factor"]
    velocity = (1 / n) * ((diameter / 4) ** (2 / 3)) * (gradient ** 0.5)
    new_df["Velocity_ms"] = velocity
    new_df["Velocity_Color"] = new_df["Velocity_ms"].apply(velocity_color)
    return new_df


def get_status_text(df: pd.DataFrame) -> tuple[str, float, float]:
...
if uploaded_file is None:
    ...
    result_df = recalculate(raw_df, diameter, gradient, flow_per_pe)
    ...
    display_df["Design_Flow_m3s"] = display_df["Design_Flow_m3s"].map(lambda v: round(float(v), 6))
    display_df["Velocity_ms"] = display_df["Velocity_ms"].map(lambda v: round(float(v), 3))
